...
- [Expandable Rows](#expandable-ows)
- [Types](#types)
...
Users can control the visibility of columns in one place using `panelOptions`. By setting a value, a panel is added above the table with a meatball icon at the right corner, and clicking on it shows a dropdown menu with columns for selection. Additionally, users can use `slot='panelGroupLeft'` and `slot='panelGroupRight'` to add custom content to the panel.

<Story id="components-data-table--column-visibility" height={'480px'} />
...
interface ModusDataTableColumnVisibility {
  title: string;
  disabledColumns?: string[];
}

interface ModusDataTablePanelOptions {
...
| Column Resize | Users can select the column for resizing by using the `Tab` key. and by pressing the `Spacebar` or `Enter` key to enable the column resize for the seleted column. To resize, use the `left`/`right` keys. When the `Enter` or `Spacebar` key is pressed, its position is saved. |
...
| `panelOptions`        | `panel-options`           | (Optional) To display a panel above the table, which allows access to table operations like hiding columns. | `ModusDataTablePanelOptions` | `null`       |
| `isExpand`            | `is-expand`               | (optional) To display expandable rows in table.                                                             | `boolean`                    | `false`      |
